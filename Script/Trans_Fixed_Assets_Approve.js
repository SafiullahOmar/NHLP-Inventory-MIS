...
function showTransferedIssuedItems(Tprov, TDept, TMonth) {
    ...
    $.ajax({
        type: "POST",
        url: "Trans_Fixed_Assets_Aprove.aspx/getTranIssuedItemsList",
        data: ParmJson,
        dataType: "json",
        ...
        success: function (data) {
            ...
            $.each(data.d, function (i, x) {
                ...
                "<td >" + count + "</td>" +
               "<td ><span style='display:none;'>" + x.ItemID + "</span><span style='display:none;'>" + x.BCode + "</span>" + x.Asset + "-"+x.BCode+"</td>" +
...
function Approve_transfer_items() {
    if (confirm('Are You Sure to Approve the Selected Transfered Items ? Once You Approved Then You Are Not Able To ROLLBACK')) {
        ...
        $.ajax({
            type: "POST",
            url: "Trans_Fixed_Assets_Aprove.aspx/ApproveTransferedItems",
            data: jsonObject,
            dataType: "json",
            async: false,
            contentType: "application/json; charset=utf-8",
