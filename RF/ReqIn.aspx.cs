...
public partial class pages_Req : System.Web.UI.Page
{
    ...
    [WebMethod]
    [System.Web.Script.Services.ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public static List<SubDepartment> GetSubDepartment(string DepartmentId)
    {
        List<SubDepartment> lst = new List<SubDepartment>();
        string constr = System.Web.Configuration.WebConfigurationManager.ConnectionStrings["INVCon"].ConnectionString;
        SqlConnection con = new SqlConnection(constr);
        SqlCommand com = con.CreateCommand();
        com.CommandType = CommandType.StoredProcedure;
        com.CommandText = "shared_getSubDepartment";
        com.Parameters.AddWithValue("@DepartmentId", SqlDbType.Int).Value = DepartmentId;
        con.Open();
        SqlDataReader dr = com.ExecuteReader();
        while (dr.Read())
        {
            SubDepartment p = new SubDepartment();
            p.SubName = dr["SubDepartment"].ToString();
            p.SubID = dr["SubDepartmentID"].ToString();
            lst.Add(p);
        }
        dr.Close();
        con.Close();
        ...
        try
        {
            ...
            if (formDetails.lst.Count > 0)
            {
                ...
                if (query.Length > 0)
                {
                    ...
                    p[3] = new SqlParameter("@SDP", SqlDbType.Int) { Value = formDetails.SubDepartment };
                    ...
                    p[8] = new SqlParameter("@ReqType", SqlDbType.Int) { Value = 3 };
